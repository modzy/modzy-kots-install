#!/bin/bash
# This modifies the KOTS installation script https://kots.io/install to pin version to 1.50.0
...
function install {
	...
	*) fail "unknown os: $(uname -s)";;
	...
	case "${OS}_${ARCH}" in
	"darwin_amd64")
		URL="https://github.com/replicatedhq/kots/releases/download/v1.50.0/kots_darwin_amd64.tar.gz"
		FTYPE=".tar.gz"
		;;
	"linux_amd64")
		URL="https://github.com/replicatedhq/kots/releases/download/v1.50.0/kots_linux_amd64.tar.gz"
		FTYPE=".tar.gz"
		;;
	"linux_amd64")
		URL="https://github.com/replicatedhq/kots/releases/download/v1.50.0/kots_linux_amd64_experimental.tar.gz"
		FTYPE=".tar.gz"
		;;
	*) fail "No asset for platform ${OS}-${ARCH}";;
	esac
	#got URL! download it...
	echo "Installing $USER/$PROG $RELEASE ($URL)..."
	#enter tempdir
	mkdir -p $TMP_DIR
	cd $TMP_DIR
	if [[ $FTYPE = ".gz" ]]; then
		...
		bash -c "$GET $URL" > "kots_${OS}_${ARCH}" || fail "download failed"
	else
